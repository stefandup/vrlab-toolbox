...
from vrlab_toolbox.processing.crane_pipeline import build_crane_participant_output_schema
from vrlab_toolbox.processing.crane_pipeline import run_pipeline as run_crane_pipeline
from vrlab_toolbox.processing.plot_utils import save_plot

logger = logging.getLogger(__name__)

# Matches sub-<id>_task-crane_acq-physiology..._physio.mat under a BIDS folder -- the
# same pattern `run_batch` globs for below, exposed here so a caller (e.g. the crane
# process-results GUI, see crane_process_results_gui.py) can discover the same subject
# roster from the BIDS folder itself without duplicating this string.
PHYSIO_GLOB_PATTERN = "sub-*_task-crane_acq-physiology*_physio.mat"
...
def run_batch(
    ...
    """Batch-processes every crane physiology recording found under `input_folder`,
    ...
    needs this callback instead. Plain callable, no Qt import here: the crane
    process-results GUI's "Process BIDS Folder" button (`crane_process_results_gui.py`)
    ...
    Shared by the `vrlab_crane_process` CLI below and that GUI button -- both call this
