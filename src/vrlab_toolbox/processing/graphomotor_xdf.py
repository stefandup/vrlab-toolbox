...
from vrlab_toolbox.cli.check_mobi_xdf import check_mobi_xdf as get_and_check_xdf
...
def summarise_xdf_candidate(xdf_fn: Path, streams: list[XdfStream]) -> dict[str, Any]:
    ...
    drawing_detected, drawing_start, drawing_end, drawing_active_duration = get_pen_drawing_window(
        pen_stream
    )
    ...
    mean_coverage = (eda_coverage + ecg_coverage + neon_coverage + eeg_coverage) / 4.0
    ...
    if drawing_detected and all(
        ...
        eda_coverage >= GOOD_COVERAGE_FRACTION and ecg_coverage >= GOOD_COVERAGE_FRACTION
    ):
        selection_class = 3
...
def find_subject_ids(root: Path) -> list[str]:
    ...
    return sorted({lsl.get_subject_id(xdf_fn) for xdf_fn in root.rglob("*.xdf")})


def find_subject_xdf_files(root: Path, subject_id: str) -> list[Path]:
    """Return all XDF candidates belonging to one participant."""
    return sorted(
        xdf_fn for xdf_fn in root.rglob("*.xdf") if lsl.get_subject_id(xdf_fn) == subject_id
    )
...
def add_qc_flags(selected_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in out.iterrows():
        ...
        if not bool(row.get("Filename_Is_Current_EEG_XDF", False)) and bool(
            row.get("Subject_Has_Any_EEG_XDF", False)
        ):
        ...
        reason = (
            "; ".join(problems) if problems else "Valid drawing with strong multimodal coverage"
        )
