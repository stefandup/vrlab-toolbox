...
from vrlab_toolbox.processing.eeg import (
    BAD_CHANNELS,
    EEGProcessingError,
    bandpower_welch,
    clean_eeg,
...
from vrlab_toolbox.processing.graphomotor_task import GraphomotorTaskResult
...
def get_drawing_intervals_from_ipad(
    ...
    drawing_intervals = [
        (start - eeg_start_time, end - eeg_start_time) for start, end in intervals_abs
    ]
...
def run_spiral_eeg_processing(
    ...
    try:
        ...
        keep_idx = [
            i for i, channel in enumerate(original_channel_names) if channel not in BAD_CHANNELS
        ]
