...
def get_event_time_with_fallback(
...
def slice_data_frame(
    timestamped_df_in: pd.DataFrame, vr_intervals: dict[str, tuple[float, float]]
...
def remove_known_false_triggers(
    trigger_interval_pairs: list[tuple[float, float]],
) -> tuple[list[tuple[float, float]], ProcessingStatus]:
...
def get_predicted_trigger_intervals(
