...
class ProcessPhysiologyDataStrategy(Protocol):
    def process_physiology_data(self, config_in : ParticipantConfig, biodata_in : RawBioData ,data_in : PipelineData | None = None) ->PipelineData:
        ...
...
class ImportBioDataError(Exception):
    """Raw biodata could not be imported"""

class ProcessBehaviourError(Exception):
    """Behaviour data could not be imported correctly"""

class ProcessPhysiologyError(Exception):
    """Physiology could not be processed successfully"""

class SavingDataError(Exception):
    """Data could not be saved successfully"""
...
class PipelineTemplate(ABC):
    ...
    def run(self, config_in : ParticipantConfig) -> None:
        ...
        try:
            pipeline_data = self.process_behav_strategy.process_behaviour_data(config_in)
        ...
        try:
            pipeline_data = self.process_physiology_strategy.process_physiology_data(config_in,raw_physiology_data,pipeline_data)
