...
* **Vibe-coding file scope**: BYPASS only ever applies to files on the "Cleared for vibe coding" list in the [AI Style Guide](docs/ai-style-guide.md#cleared-for-vibe-coding). Everything else in `src/mooi_toolbox/processing/` is the "Verboten without OVERRIDE" list on that same page — it needs OVERRIDE, not BYPASS, and OVERRIDE doesn't waive [AI Use Guidelines](docs/ai-use.md)'s "genuinely unavoidable" bar or the understand/explain/extend test. Same exact-scope-only and same-session-only rules as BYPASS scope above apply to OVERRIDE.
