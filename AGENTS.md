...
* **BYPASS: <what to do>** → skip the interaction loop and do exactly what's named — nothing more. Only covers files listed in [`vibe_list.md`](vibe_list.md) (the repo-root allow-list; rendered at [AI Style Guide](docs/ai-style-guide.md#cleared-for-vibe-coding) too — BIDS conversion, dummy/sample-data generation, CLIs, the crosscheck feature, the process results viewer, the toolbox launcher, build/packaging scripts, docs); for anything not on that list — anywhere in the repo, not just `src/vrlab_toolbox/processing/` — use OVERRIDE instead. Once invoked, bypass stays in effect for the rest of the *current session* (no need to repeat it each message), but never carries over into a new or different session — those always start with the full loop. It ends early if the user says RESUME LOOP, or if AGENTS.md gets reinvoked mid-session (the user quoting or stating it again) — either resets to the full loop. Bypass doesn't imply permission for extra edits beyond each named scope; if a new step's scope is ambiguous, ask before proceeding rather than assuming it's covered.
* **OVERRIDE: <what to do>** → same mechanics as BYPASS (skip the loop, do exactly what's named, same session-only persistence, same RESUME LOOP / AGENTS.md-reinvocation reset), but for any file *not* listed in [`vibe_list.md`](vibe_list.md) — see [AI Style Guide](docs/ai-style-guide.md#verboten-without-override). That's default-deny, not a separately maintained verboten list: today it's almost entirely the pipeline/processing/schema logic [AI Use Guidelines](docs/ai-use.md#the-core-rule-ai-is-a-tutor-not-a-coder) says to hand-write, but a brand-new file requires OVERRIDE the moment it exists, with nothing to add anywhere to make that true — only adding it *to* `vibe_list.md`'s allow-list ever flips it to BYPASS-eligible instead. Using OVERRIDE doesn't relax the hand-write standard: it's for the "genuinely unavoidable" case that page already allows for, not a way around it, and whatever lands still has to pass the [four rules of vibe coding](docs/ai-use.md#the-four-rules-of-vibe-coding). Same exact-scope-only rule as BYPASS — it authorizes only what's named, nothing more.
* **RESUME LOOP** → turn the interaction loop back on for the rest of the current session (cancels an active BYPASS or OVERRIDE).
...
* **Restricted-folders guardrail (critical, absolute — not affected by BYPASS or OVERRIDE)**: Check [`.ai_restricted`](.ai_restricted) (repo root) before reading or writing anywhere in the repo. It's a plain-text, human-maintained list of folders — one per line, `.gitignore`-style — that this assistant must never open, read, write to, or otherwise access, for any reason. This is where the user stores example/sample data they don't want an AI touching. BYPASS/OVERRIDE only skip the *interaction loop*; they never lift this guardrail. `.ai_restricted` itself is covered by the same restriction: read it to know what's listed, but never create, edit, or delete it — only a human maintains it.

* Never claim to have run code, tests, or linters. Only *suggest* commands — except the dummy-data generation CLIs (`vrlab_crane_generate_sample_data` and siblings; see [AI Style Guide](docs/ai-style-guide.md#execution-exception-dummy-data-generation)), running the `tests/` suite via `pytest` (see the Data guardrail exception above), running the project's configured code-checking tools (`ruff check`, `ruff format`, `pyright`/Pylance) — including fixing whatever they flag — against files listed in [`vibe_list.md`](vibe_list.md) (see [AI Style Guide](docs/ai-style-guide.md#execution-exception-code-checking-tools)), and running the build scripts `build.ps1`/`build_mac.sh` (see [AI Style Guide](docs/ai-style-guide.md#execution-exception-build-scripts)), all of which may be run directly. Dummy-data generation only ever writes synthetic data, so the human step there moves to reviewing the generated output afterward rather than approving the command beforehand; running tests only ever produces pass/fail results, not new data; running/fixing a linter or type-checker on an already-BYPASS-eligible file is the same authorship permission BYPASS already grants, just applied to catching what it missed; the build scripts only ever write into the gitignored `build_output/` folder (plus, for a PyInstaller build, a local editable-install of this same package into the active venv) and never touch git or any remote system. This exception doesn't extend to any other command, or to any file not on `vibe_list.md` — running the BIDS-conversion CLIs standalone, the crosscheck tool, any pipeline/processing CLI (e.g. `vrlab_crane_process`), a code-checker against an OVERRIDE-only file, or `toolbox_installer.iss`/ISCC.exe directly (outside of `build.ps1` invoking it) still means only suggesting the command.
...
* **Vibe-coding file scope**: BYPASS only ever applies to files listed in [`vibe_list.md`](vibe_list.md) (rendered at [AI Style Guide](docs/ai-style-guide.md#cleared-for-vibe-coding)). Everything else, anywhere in the repo — not just `src/vrlab_toolbox/processing/` — is default-deny (see [AI Style Guide](docs/ai-style-guide.md#verboten-without-override)): it needs OVERRIDE, not BYPASS, and OVERRIDE doesn't waive [AI Use Guidelines](docs/ai-use.md)'s "genuinely unavoidable" bar or the understand/explain/extend test. Same exact-scope-only and same-session-only rules as BYPASS scope above apply to OVERRIDE.
