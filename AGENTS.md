...
* **Vibe-coding file scope**: BYPASS only ever applies to files listed in [`vibe_list.md`](vibe_list.md) (rendered at [AI Style Guide](docs/ai-style-guide.md#cleared-for-vibe-coding)). Everything else, anywhere in the repo — not just `src/mooi_toolbox/processing/` — is default-deny (see [AI Style Guide](docs/ai-style-guide.md#verboten-without-override)): it needs OVERRIDE, not BYPASS, and OVERRIDE doesn't waive [AI Use Guidelines](docs/ai-use.md)'s "genuinely unavoidable" bar or the understand/explain/extend test. Same exact-scope-only and same-session-only rules as BYPASS scope above apply to OVERRIDE.
